...
var getEntry = function (opt) {
    ...
    var entry_prod = {
        index: [
            ...
            'webpack-dev-server/client?' + opt.config.p_url,
    ...
    var bash = opt.bash || './src/**/*.js';
    glob.sync(bash)
        .forEach(function (name) {

            var start = name.indexOf(root) + 4,
                end = name.length - 3;
            var n = name.slice(start, end);
            n = n.slice(0, n.lastIndexOf('/'));
            //保存各个组件的入口
            entry[n] = ['webpack/hot/dev-server',
                'webpack-dev-server/client?' + opt.config.p_url, name
            ];
        });
    //console.log(opt.prod);
    return entry;
};
webpackConfig = {
    init: function (opt) {
